...
def test_random_log_price_interior_points_shape_and_ranges() -> None:
    X = make_random_log_price_interior_points(
        n=10,
        t_min=0.1,
        t_max=0.8,
        S_min=60.0,
    ...
    assert np.all(X[:, 0] >= 0.1)
    assert np.all(X[:, 0] <= 0.8)
    assert np.all(X[:, 1] >= np.log(60.0))
    assert np.all(X[:, 1] <= np.log(140.0))


def test_random_log_price_terminal_boundary_shape_and_ranges() -> None:
    X = make_random_log_price_terminal_boundary(
        n=8,
        maturity=1.0,
        S_min=60.0,
        S_max=140.0,
...
def test_random_log_price_lower_boundary_shape_and_ranges() -> None:
    X = make_random_log_price_lower_boundary(
        n=7,
        t_min=0.2,
        maturity=1.0,
        S_min=60.0,
...
def test_random_log_price_upper_boundary_shape_and_ranges() -> None:
    X = make_random_log_price_upper_boundary(
        n=7,
        t_min=0.2,
        maturity=1.0,
        S_max=140.0,
...
def test_random_log_price_grid_same_seed_reproduces_grid() -> None:
    kwargs = {
        "n_int": 6,
        "n_terminal": 5,
        "n_lower": 3,
    ...
    first = make_random_log_price_grid(**kwargs)
    second = make_random_log_price_grid(**kwargs)
