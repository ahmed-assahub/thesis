"""Black-Scholes collocation-size sensitivity experiment."""
...
class ExperimentConfig:
    """Configuration for the Black-Scholes collocation sensitivity experiment."""
    ...
    n_int_values: tuple[int, ...] = (200, 400, 600)
...
def run_single_case(
    ...
    """Run one maturity and collocation configuration."""
    ...
    train_grid = make_grid(config, maturity, n_int, n_bd, train_seed)
    tune_grid = make_grid(config, maturity, n_int, n_bd, tune_seed)
...
def run_experiment(
    ...
    """Run all configured Black-Scholes collocation sensitivity cases."""

    aggregate_rows: list[dict[str, Any]] = []
    maturity_rows: list[dict[str, Any]] = []
    hyperparameter_rows: list[dict[str, Any]] = []
    config_pairs = _collocation_pairs(config)
    for (n_int, n_bd) in config_pairs:
        group_predictions: list[np.ndarray] = []
        group_references: list[np.ndarray] = []
        group_spots: list[np.ndarray] = []
        ...
        for maturity_index, maturity in enumerate(config.maturities):
            ...
            group_rows.append(maturity_row)
            group_predictions.append(arrays["pred"])
            group_references.append(arrays["ref"])
            group_spots.append(arrays["S0"])
        aggregate_rows.append(
            _aggregate_row(
                config=config,
                n_int=n_int,
                n_bd=n_bd,
                maturity_rows=group_rows,
                predictions=np.concatenate(group_predictions),
                references=np.concatenate(group_references),
                spots=np.concatenate(group_spots),
            )
        )
