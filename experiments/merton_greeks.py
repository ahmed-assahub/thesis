"""Merton jump-diffusion GPR Greek accuracy experiment."""
...
class ExperimentConfig:
    """Configuration for the Merton GPR Greek experiment."""
    ...
    output_dir: Path = Path("results/merton_jump_diffusion/greeks")
...
def make_boundary_value_fn(config: ExperimentConfig, maturity: float):
    """Return Merton call boundary values on the finite domain."""
...
def run_single_case(
    ...
    price_ref = merton_jump_call_price_log(
        X_star,
        strike=config.strike,
        maturity=maturity,
        r=config.r,
    ...
    delta_ref, gamma_ref, theta_ref = merton_jump_call_greeks_log(
        X_star,
        strike=config.strike,
        maturity=maturity,
        r=config.r,
    ...
    prediction_rows = _prediction_rows(
        model_name="merton_jump_diffusion",
        maturity=maturity,
        S0=S0,
        x0=x0,
        strike=config.strike,
...
def run_experiment(
    ...
    """Run all configured Merton Greek cases."""
...
def _config_json(config: ExperimentConfig) -> dict[str, Any]:
    ...
    data["greek_convention_note"] = (
        ...
        "maturity. Reference Greeks are differentiated Merton series values. "
        "No finite differences are used in experiment computations."
    )
    data["error_reference_note"] = (
        "Price and Greek errors are computed against Merton semi-analytical "
        "series benchmarks."
    )
    return data
